...
```{r, APA table for ranked estimates}

# Read the CSV file with MNI coordinates
mni_data_ranked <- read_csv("~/Downloads/mni_coordinates_100_channels.csv")

# Clean up and rename coordinate columns
mni_coords_ranked <- mni_data_ranked %>%
  mutate(across(c(X, Y, Z), ~ round(.x, 2))) %>%
  rename(
    Channel = Channel,
    X_ranked = X,
...
# Extract results for the interaction term from the rank-based models
interaction_term_results_ranked <- rank_model_results %>%
  filter(term == "shared_exp_rank:ADHDscore_rank") %>%
  mutate(
    Channel = as.numeric(gsub("ch", "", channel)),
    t_ranked = round(statistic, 2),
    p_ranked = round(p.value, 4),
    q_ranked = round(p_adj_fdr, 4)
  ) %>%
  select(Channel, t_ranked, p_ranked, q_ranked)

# Merge with ranked MNI coordinates
final_results_ranked <- left_join(interaction_term_results_ranked, mni_coords_ranked, by = "Channel")

# Create an APA-formatted flextable
apa_table_ranked <- flextable(final_results_ranked) %>%
  set_header_labels(
    Channel = "Channel",
    t_ranked = "t",
    p_ranked = "p",
    q_ranked = "q (FDR corrected)",
    X_ranked = "X (MNI)",
    Y_ranked = "Y (MNI)",
    Z_ranked = "Z (MNI)"
  ) %>%
...
# Print the table
print(apa_table_ranked)

```

```{r, checking for Cronbach's alpha for ASRS and TAS}

# Select ADHD item columns
ADHD_items <- shared_exp[, c("ADHD_1", "ADHD_2", "ADHD_3", "ADHD_4", "ADHD_5", "ADHD_6",
                             "ADHD_7", "ADHD_8", "ADHD_9", "ADHD_10", "ADHD_11", "ADHD_12",
                             "ADHD_13", "ADHD_14", "ADHD_15", "ADHD_16", "ADHD_17", "ADHD_18")]

# Select TAS item columns
TAS_items <- shared_exp[, c("TAS_1", "TAS_2", "TAS_3", "TAS_4", "TAS_5", "TAS_6", "TAS_7",
                            "TAS_8", "TAS_9", "TAS_10", "TAS_11", "TAS_12", "TAS_13", "TAS_14",
                            "TAS_15", "TAS_16", "TAS_17", "TAS_18", "TAS_19", "TAS_20")]

# Remove rows with any missing values in ADHD and TAS items
ADHD_clean <- na.omit(ADHD_items)
TAS_clean <- na.omit(TAS_items)

cronbach.alpha(ADHD_clean, CI = TRUE, standardized = FALSE)
cronbach.alpha(TAS_clean, CI = TRUE, standardized = FALSE)
